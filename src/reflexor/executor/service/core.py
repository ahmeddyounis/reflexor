...
class ExecutorService:
    ...
    async def execute_task(self, task_id: str) -> ExecutionReport:
        ...
        with start_span(
            ...
            attributes={"task.id": normalized_task_id},
        ):
            ...
            with correlation_context(
                ...
                async def on_before_execute() -> None:
                    ...
                    if (
                        ...
                        and tool_call.status == ToolCallStatus.RUNNING
                    ):
                        return
                ...
                async with self._limiter.limit(tool_call.tool_name):
                    ...
                    with start_span(
                        ...
                        attributes={
                            ...
                            "tool_call.id": tool_call.tool_call_id,
                            "tool.name": tool_call.tool_name,
                            "tool.permission_scope": tool_call.permission_scope,
                        },
                    ):
    ...
    async def process_lease(self, lease: Lease) -> ExecutionReport:
        ...
        trace_carrier = _trace_carrier_from_lease(lease)
        ...
        with start_span(
            ...
            attributes={
                "task.id": task_id,
                "run.id": lease.envelope.run_id,
                "queue.attempt": lease.envelope.attempt,
                "queue.reason": _trace_field(lease.envelope.trace, "reason"),
                "queue.source": _trace_field(lease.envelope.trace, "source"),
            },
        ):
            ...
            with correlation_context(
                ...
                if task.status in {
                    TaskStatus.PENDING,
                    TaskStatus.QUEUED,
                } and has_unmet_dependencies(task=task, all_tasks=run_tasks):
...
def _trace_carrier_from_lease(lease: Lease) -> dict[str, str] | None:
    trace_payload = lease.envelope.trace
