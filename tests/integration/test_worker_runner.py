...
from reflexor.infra.db.models import Base, TaskRow, ToolCallRow
...
class _SignalTool:
    manifest = ToolManifest(
        name="tests.worker_tool",
        version="0.1.0",
        description="Tool used by worker runner integration test.",
...
async def _sqlite_file_session_factory(
    ...
    db_path = tmp_path / "reflexor_worker_runner_test.db"
...
@pytest.mark.asyncio
async def test_worker_runner_processes_task_and_shuts_down(tmp_path: Path) -> None:
    called = asyncio.Event()
    tool = _SignalTool(called=called)
    registry = ToolRegistry()
